...
const ChatDrawer = ({ open, onClose, openSupport = false }: any) => {
  // ✅ ALL hooks must be called unconditionally at the top — no early returns before this
  const { user } = useAuth();
  ...
  // ✅ Conditional render happens AFTER all hooks are declared
  if (!open) return null;
      ...
      >
        <div className="p-4 bg-indigo-600 text-white flex items-center justify-between">
          <div className="flex items-center gap-2">
            {selectedId && (
              <ChevronLeft className="cursor-pointer" onClick={() => setSelectedId(null)} />
            )}
            ...
            <ScrollArea className="h-full">
              {conversations.map((c: any) => (
                <div
                  ...
                  className="p-4 border-b hover:bg-slate-50 cursor-pointer flex items-center gap-3"
                >
                  <Avatar>
            ...
            <div className="flex flex-col h-full">
              <ScrollArea className="flex-1 p-4 bg-slate-50">
                {messages.map((m: any) => (
                  <div
                    key={m.id}
                    className={`mb-4 flex ${m.sender_id === user?.id ? "justify-end" : "justify-start"}`}
                  >
                    <div
