...
const PropertyDetail = () => {const { id } = useParams<{ id: string }>();const navigate = useNavigate();const { user } = useAuth();const [p, setP] = useState<any>(null);const [loading, setLoading] = useState(true);const [activeImg, setActiveImg] = useState(0);const [showVR, setShowVR] = useState(false);const [hasRequested, setHasRequested] = useState(false);const [isFavorite, setIsFavorite] = useState(false);const [requestLoading, setRequestLoading] = useState(false);useEffect(() => {const getData = async () => {if (!id) return;
  ...
  const startChat = () => {
    ...
    if (!p) return;
    if (user.id === p.landlord_id) return toast.info("This is your own property");
    navigate(`/chat?with=${p.landlord_id}&property=${p.id}`);
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-[80px,1fr,400px] gap-6 md:gap-8 items-start mb-20">
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <button
                onClick={startChat}
                className="flex items-center justify-center gap-2 py-4 rounded-2xl bg-secondary font-black text-[10px] uppercase tracking-widest hover:bg-secondary/80 transition-all active:scale-95"
              >
                <MessageCircle className="w-4 h-4" /> Chat Now
              </button>
              <button
                onClick={() => {
                  if (!user) return toast.error("Please sign in to schedule");
                  if (user.id === p.landlord_id) return toast.info("This is your own property");
                  setShowVisitDialog(true);
                }}
                className="col-span-2 flex items-center justify-center gap-2 py-4 rounded-2xl bg-primary text-primary-foreground font-black text-[10px] uppercase tracking-widest hover:brightness-110 active:scale-95 transition-all"
              >
