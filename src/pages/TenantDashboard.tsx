  ...
  const PropertyCard = ({ p, isHero = false }: { p: any, isHero?: boolean }) => {
    ...
    return (
      <motion.div 
        ...
        className={`bg-card rounded-[2.5rem] overflow-hidden border ${isHero ? 'border-primary/40 shadow-2xl shadow-primary/10' : 'border-border/50'} group cursor-pointer hover:shadow-2xl transition-all`} 
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="relative">
              <button onClick={() => setShowUserMenu(!showUserMenu)} className="flex items-center gap-2 bg-card p-1 pr-3 rounded-full border border-border/50">
                <img src={userProfile?.avatar_url || `https://api.dicebear.com/7.x/avataaars/svg?seed=${userProfile?.full_name || 'User'}`} className="w-8 h-8 rounded-full border" />
                <ChevronDown className="w-3 h-3" />
              </button>
              {/* User Menu logic omitted for brevity, keep your existing one */}
            </div>
          </div>
